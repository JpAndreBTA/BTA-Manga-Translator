"""
manga_translator.py
BTA MangaTranslate — manga translator with character memory.
...
FORCE_CPU = os.environ.get("BTA_FORCE_CPU", "").strip().lower() in {"1", "true", "yes", "on"}
...
PROJECT_DIR = os.path.dirname(os.path.abspath(__file__))
HF_CACHE_DIR = os.environ.get("HF_HOME") or os.path.join(PROJECT_DIR, "models", "huggingface")
HF_HUB_CACHE_DIR = os.environ.get("HUGGINGFACE_HUB_CACHE") or os.path.join(HF_CACHE_DIR, "hub")
...
# deckyfx/anime-big-lama is no longer public/reachable; df1412 is the working
# Hugging Face mirror currently linked from the project README.
LAMA_REPOS = [
...
def save_output_image(output_path: str, img_cv: np.ndarray) -> str:
    """Save rendered pages as compact WebP by default, preserving alpha when present."""
    ext = os.path.splitext(output_path)[1].lower()
    params = [cv2.IMWRITE_WEBP_QUALITY, WEBP_QUALITY] if ext == ".webp" else []
    ok = cv2.imwrite(output_path, img_cv, params)
    if not ok:
...
def hf_token():
    ...
    for filename in ("huggingface_token.txt", ".env", ".env.local"):
        token = _read_hf_token_file(os.path.join(PROJECT_DIR, filename))
        if token:
...
def hf_cache_dirs():
    fallback = os.path.expanduser(os.path.join("~", ".cache", "huggingface", "hub"))
    dirs = []
    for path in (HF_HUB_CACHE_DIR, fallback):
...
def cached_hf_download(repo_id, filename):
    for cache_dir in hf_cache_dirs():
        try:
            ...
            print(f"[hf] Using cached {repo_id}/{filename}")
            return model_path
        except Exception:
            pass

    print(f"[hf] Cache missing for {repo_id}/{filename}; downloading once...")
...
def _tensor_to_device(value):
    if not torch.is_tensor(value):
        return value
    value = value.to(DEVICE, non_blocking=USE_CUDA)
    if USE_CUDA_FP16 and value.is_floating_point():
        value = value.half()
    return value


def _batch_to_device(batch: dict) -> dict:
    return {key: _tensor_to_device(value) for key, value in batch.items()}
...
def load_inpainting_model():
    ...
    """
    if DISABLE_LAMA:
        print("[lama] Disabled by KOTOBA_DISABLE_LAMA=1, using cv2.inpaint fallback")
        return None
    ...
    for repo_id, filename in LAMA_REPOS:
        try:
            ...
            model_path = cached_hf_download(repo_id, filename)
...
def load_detector():
    processor, model = load_hf_transformers_pair(BUBBLE_MODEL_ID)
    model.to(DEVICE)
    if USE_CUDA_FP16:
        model.half()
    model.eval()
    print(f"[detector] Device: {DEVICE} | fp16={'on' if USE_CUDA_FP16 else 'off'}")
    return processor, model
...
def translate_batch(bubbles: list[dict], page_context: str,
    ...
    CHUNK_SIZE = max(5, int(os.environ.get("BTA_TRANSLATION_CHUNK_SIZE", "12")))
    ...
    for chunk_idx, chunk in enumerate(chunks):
        _translate_chunk(
            ...
            page_context, manga_ctx, target_lang, source_lang, gender_hints,
    ...
    if missing_indices:
        ...
        for seq in missing_indices:
            ...
            translation = _translate_persistent(
                b, page_context, manga_ctx, target_lang, source_lang, gender_hints
            )
            if translation:
...
def _translate_chunk(chunk: list, chunk_idx: int, to_translate: list,
                     ...
                     source_lang: str, gender_hints: dict, errors,
                     page_idx: int, retries: int) -> None:
    """
    ...
    source_hint = (
        "The source language is unknown; infer it from the OCR text."
...
def _translate_persistent(bubble: dict, page_context: str,
                          manga_ctx: MangaContext, target_lang: str,
                          source_lang: str, gender_hints: dict) -> str:
    """
    ...
    source_hint = (
        "Source language: infer from the text."
        if not source_lang or source_lang.lower() == "auto"
        else f"Source language: {source_lang}."
    )
...
def detect_bubbles(image_pil: Image.Image, threshold: float = 0.5) -> list[dict]:
    ...
    inputs = _batch_to_device(processor(images=image_pil, return_tensors="pt"))
    with torch.no_grad():
...
def inpaint_page(img_cv: np.ndarray, bubbles: list[dict]) -> np.ndarray:
    ...
    try:
        ...
        # 2. Run LaMa in FP32. Its FFT layers can fail in CUDA fp16 for non power-of-two page sizes.
        with torch.no_grad():
...
def fit_text_in_box(draw, text: str, box_w: int, box_h: int,
    ...
    font = try_load_font(4) or ImageFont.load_default()
    words = text.split() or [text]
    ...
    if hasattr(font, "getmetrics"):
        ...
        ascent, descent = 6, 2
    lh = max(6, ascent + descent)
    return font, lines, [lh] * len(lines), 1
...
def attach_render_boxes(text_bubbles: list[dict], all_bubbles: list[dict],
    ...
    for tb in text_bubbles:
        ...
        for parent in containers:
            ...
            if pw * ph <= w * h:
                continue
            contains_center = px <= cx <= px + pw and py <= cy <= py + ph
            ix0, iy0 = max(x, px), max(y, py)
            ix1, iy1 = min(x + w, px + pw), min(y + h, py + ph)
            overlap = max(0, ix1 - ix0) * max(0, iy1 - iy0)
            if contains_center or overlap >= (w * h * 0.35):
...
def process_page(image_path: str, page_idx: int,
    ...
    print(f"Languages: {source_lang or 'Auto'} -> {target_lang}")
    if fast_mode:
    ...
    text_bubbles = attach_render_boxes(text_bubbles, bubbles, image_pil.size[0], image_pil.size[1])
    ...
    if not fast_mode and len(text_bubbles) <= 2:
        ...
        if detect_character_intro_page(image_path):
            ...
            if render_output:
                save_output_image(output_path, img_cv)
                print(f"\nSaved (unchanged): {output_path}")
            else:
                print("\nOverlay mode: image rendering skipped")
    ...
    translate_batch(text_bubbles, page_context, manga_ctx, target_lang,
                    source_lang=source_lang, errors=errors, page_idx=page_idx)
    for i, b in enumerate(text_bubbles):
    ...
    if font_size:
        ...
        for b in text_bubbles:
            b["font_size"] = safe_font_size

    if render_output:
        stage("stage_inpaint")
        annotated = draw_results(img_cv, text_bubbles, debug=debug)
        save_output_image(output_path, annotated)
...
def process_directory(input_dir: str, output_dir: str = RESULTS_DIR,
    ...
    for page_idx, filename in enumerate(files, start=1):
        ...
        output_path = os.path.join(output_dir, f"{name}_translated.webp")
        ...
        try:
            bubbles_result = process_page(
                ...
                output_path, target_lang, source_lang=source_lang, debug=debug,
                font_size=font_size, fast_mode=fast_mode, errors=errors,
                on_stage=on_stage,
            ) or []
